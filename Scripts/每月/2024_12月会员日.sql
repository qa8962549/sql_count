...
where 1=1
and event_time >='2024-12-20' 
and `date` >= '2024-12-20' 
and `date` < '2025-01-01'
and event='Page_entry'
--and page_title ='沃尔沃汽车双旦会员日'
...
from
(-- 
	select gio_id,distinct_id,toDateTime(left(`time`,19)) as `time`,memberid,date
	from dwd_23.dwd_23_gio_tracking a
	where 1=1
	and event_time>'2024-12-20'
	and `date` >= '2024-12-20' 
...
from 
	...
	join (
		...
		where 1=1
		and event_time >='2024-12-20' 
		and a.`date` >= '2024-12-20' 
		and `date` < '2025-01-01'
		and length(a.distinct_id)<9
		and event = 'Page_entry'
--		and page_title='沃尔沃汽车双旦会员日'
--		and page_title='沃尔沃汽车双旦会员日二级页面'
		and var_activity_name='2024年12月沃尔沃汽车双旦会员日'
		and ((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App')
	)x on toString(x.distinct_id) =toString(m.cust_id) 
	where tir.is_deleted=0
	and date(tir.create_time)>= '2024-12-20' 
	and date(tir.create_time)< '2025-01-01'
...
--		and page_title='沃尔沃汽车双旦会员日'
	...
	WHERE 1=1
	and date(ta.CREATED_AT) >= '2024-12-20' 
	and date(ta.CREATED_AT) < '2025-01-01'
...
where 1=1
and toDate(a.create_time) >='2024-12-20' 
and toDate(a.create_time) < '2025-01-01'
and campaign_code in
			...
			and date(concat(left(`_data_inlh_date`,4),'-',right(left(`_data_inlh_date`,6),2),'-',right(`_data_inlh_date`,2))) = '2024-11-25'
				)
group by 1  with rollup 
order by 1 
...
--SUM(case when m.fl='充电专区' or m.fl='精品' or m.fl='售后养护' then m.`不含税的总金额` else null end) `活动GMV合计`
SUM(case when m.fl='精品' then m.`不含税的总金额` else null end) `精品（元）`,
SUM(case when m.fl='售后养护' then m.`不含税的总金额` else null end) `售后（元）`,
SUM(case when m.fl='充电专区' then m.`不含税的总金额` else null end) `充电（元）`,
SUM(case when m.fl='生活服务' then m.`不含税的总金额` else null end) `第三方（元）`,
SUM(case when m.fl='充电专区' or m.fl='精品' or m.fl='售后养护' or  m.fl='生活服务' then m.`不含税的总金额` else null end) `活动GMV合计`
from 
	(-- 1、商城订单明细(CK)
select
a.order_code `订单号`,
...
left join (
		...
		where m.rk=1) h1 on toString(a.user_id) = toString(h1.id)   -- 会员表(获取会员信息)
--join (
--			select distinct distinct_id
--			from dwd_23.dwd_23_gio_tracking a
--			where 1=1
...
--			and var_activity_name='2024年12月沃尔沃汽车双旦会员日'
--			and ((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App')
--		)x on toString(x.distinct_id) =toString(h1.cust_id) 
...
where 1=1
and a.create_time >= '2024-12-20' 
and a.create_time < '2025-01-01'  -- 订单时间
and a.is_deleted <> 1
and b.is_deleted <> 1
...
where 1=1
and event_time >='2024-12-20' and `date` >= '2024-12-20' and `date` < '2025-01-01'
and event= '$AppStart'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or  channel='App')
union all 
-- APP当日下载量
...
and date(min_app_time)<= '2024-11-25'
and is_owner =1 -- 筛选车主
union all
--活动当日APP日活
...
(-- 
	...
	where 1=1
	and event_time>'2024-12-20'
	and `date` >= '2024-12-20' and `date` < '2025-01-01'
	and ((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App')
	and event='Page_entry'
	and page_title ='沃尔沃汽车双旦会员日'
	and var_activity_name = '2024年12月沃尔沃汽车双旦会员日'
...
--		and `date` <= '2024-05-31'
		and event= 'Page_entry'
		and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` ='MiniProgram' or channel ='Mini'))
		and page_title ='9月会员日'
		and var_activity_name = '2024年9月会员日'
--		and m.is_vehicle='1'
		and length(distinct_id)<9 
	) last_month
left join (
...
and page_title ='沃尔沃汽车双旦会员日'
and var_activity_name = '2024年12月沃尔沃汽车双旦会员日'
and event_time >='2024-12-20' and a.`date` >= '2024-12-20' and `date` < '2025-01-01'
...
where tir.is_deleted=0
and date(tir.create_time)= '2024-11-25'
and tir.be_invite_member_id is not null 
...
FROM ods_invi.ods_invi_tm_invite_record_d tir  
where 1=1
and toDate(tir.create_time)>='2024-12-20'
...
--	count(m.member_phone) `预约试驾活动留资量`,
	...
	WHERE 1=1
	and date(ta.CREATED_AT) = '2024-11-25'
	AND ta.APPOINTMENT_TYPE in (70691002,70691001)      -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and ta.CREATED_AT >= '2024-12-20' -- 修改月份
	and ta.CREATED_AT < '2024-12-20' -- 修改月份
	AND ta.APPOINTMENT_TYPE in (70691002,70691001)      -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
(-- 养修预约		
	...
	join (
			...
			from dwd_23.dwd_23_gio_tracking a
			where 1=1
			and event_time > '2024-09-01' and a.`date` = '2024-09-25'
			and distinct_id not like '%#%'
			and length(a.distinct_id)<9
			and event = 'Page_entry'
			and page_title='9月会员日'
	...
	where 1=1
	and tam.IS_DELETED <>1
	and date(ta.CREATED_AT)= '2024-09-25'
	and ta.DATA_SOURCE ='C'
	and ta.APPOINTMENT_TYPE =70691005
	)ta
join
(-- 回厂
	...
	and date(o.RO_CREATE_DATE) = '2024-09-25'
...
--		and i.is_deleted ='0' -- 该表该字段全为空
	...
	where 1=1
	and toDate(a.create_time) = '2024-11-25' 
...
where 1=1
and event_time >='2024-12-20' 
and a.`date` >= '2024-12-20' and `date` < '2025-01-01'
and length(a.distinct_id)<9
and event = 'Page_entry'
...
where date(c.create_time)='2024-11-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-11-25'
	group by member_id
...
select r.id, 
r.`activity_card_id`,
r.`activity_card_code`,
c.`card_name`,r.member_id ,
r.`create_time` 
from ods_voam.ods_voam_activity_card_d c, ods_voam.ods_voam_activity_card_record_d r
where c.id = r.`activity_card_id` 
and DATE_FORMAT(r.create_time ,'%Y-%m-%d')='2024-11-25' 
and r.activity_card_code ='2024_35_memberDay_activity'; -- 十一月会员日补签
	
--test
select count(1) 
from ods_voam.ods_voam_activity_card_record_d r
where DATE_FORMAT(r.create_time ,'%Y-%m-%d')='2024-11-25' 
and r.activity_card_code <>'2024_33_memberDay_activity';
...
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title='沃尔沃汽车双旦会员日'
and page_title in ('沃尔沃汽车双旦会员日','沃尔沃汽车双旦会员日二级页面')
--and var_activity_name='2024年12月沃尔沃汽车双旦会员日'
and event ='Button_click'
--and m.is_vehicle='1' -- 车主
...
and `date` >= '2024-12-20' and `date` < '2025-01-01'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title like '%沃尔沃汽车双旦会员日%'
and page_title in ('沃尔沃汽车双旦会员日','沃尔沃汽车双旦会员日二级页面')
--and var_activity_name in ('2024年12月沃尔沃汽车双旦会员日','2024年会员日组队活动')
and event ='Button_click'
group by 1  with rollup
order by 1
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and event_time >='2024-12-20' and `date` >= '2024-12-20' and `date` < '2025-01-01'
	and page_title ='沃尔沃汽车双旦会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
and date(date_create) = '2024-11-25'
...
where a.event_code ='group'
and a.is_deleted =0
and date(a.create_time) ='2024-09-25'
order by 5 desc 
)x
...
(
	...
	and var_activity_name='2024年12月沃尔沃汽车双旦会员日'
	and event ='Page_entry'
	and var_promotion_supplement in ('4c291324y10f',
'1a291482j46p',
'1w291575h56d',
'3z291682j42h',
'5f29173x54s',
...
-- 优惠券核销情况
	...
	where 1=1
	and date(a.get_date)= '2024-11-25'
	and a.is_deleted=0 
...
from 
	...
	where 1=1
	and a.lottery_code like '%memberday-2410%'  -- 当月会员日code
	and date(a.create_time)='2024-11-25'
	and a.have_win = 1   -- 中奖
	and b.is_deleted=0
	--and prize_name like '%2024%'
	order by a.create_time
)x group by 1  with rollup
order by 1 desc 

select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-11-25'
...
and date(r.CREATE_TIME) = '2024-11-25'
order by 1
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='1沃尔沃汽车双旦会员日'
	and var_activity_name='2023年1沃尔沃汽车双旦会员日'
	and ((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App')
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-11-25' 
