...
where 1=1
and toDate(a.create_time) = '2024-06-25' 
...
--					and distinct_id not like '%#%'
	...
	(-- 浏览过活动页的 每月每人的一键留资线索
		...
		where a.create_time >= '2024-04-25'  and a.create_time < '2024-05-25'
...
and date(min_app_time)<= '2024-06-25'
and is_owner =1 -- 筛选车主
--union all
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and month(ta.CREATED_AT) = '6' -- 修改月份
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
(-- 回厂
	...
	and date(o.RO_CREATE_DATE) = '2024-06-25'
...
from
	...
	and date(a.create_time)='2024-06-25'
)x 
...
where date(c.create_time)='2024-06-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-06-25'
	group by member_id
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and `date` = '2024-06-25'
	and page_title ='6月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
(
	...
	and var_activity_name='2024年6月会员日'
	and event ='Page_entry'
	and var_promotion_activity='240613_vip_day'
--	and m.is_vehicle='1'
--	and m.is_vehicle='0'
...
-- 优惠券核销情况
	...
	where 1=1
	and date(a.get_date)= '2024-06-25'
	and a.is_deleted=0 
	and a.coupon_id in (
...
from 
	...
	where 1=1
	and a.lottery_code like 'memberday-2406'  -- 当月会员日code
	and date(a.create_time)='2024-06-25'
	and a.have_win = 1   -- 中奖
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-06-25'
...
-- 抽奖明细
	...
	left join volvo_online_activity_module.lottery_play_init lpi on lpi.lottery_play_code =a.lottery_play_code 
	left join `member`.tc_member_info d on a.member_id = d.ID
	where a.lottery_code like '%memberday-2406%'  -- 当月会员日code
	and date(a.create_time)='2024-06-25'
	and a.have_win = 1   -- 中奖
	--and prize_name like '%2024%'
	order by a.create_time
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='16月会员日'
	and var_activity_name='2023年16月会员日'
	and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-06-25' 
