...
where 1=1
and event_time > '2024-07-01' and `date` = '2024-07-25'
and event='Page_entry'
and page_title ='7月会员日'
and var_activity_name = '2024年7月会员日'
...
-- 活动拉新（APP+小程序）(点击活动后，前后十分钟注册成为了新会员)
select
...
from 
	...
	and date(tir.create_time)= '2024-07-25'
...
--	and date(tir.create_time)<='2024-06-01'
	and tir.be_invite_member_id is not null 
)x
...
(-- 到店数大于线索数时,取线索数作为到店数
	...
	(-- 浏览过活动页的 每月每人的一键留资线索
		...
		where a.create_time >= '2024-04-25'  
		and a.create_time < '2024-05-25'
...
and date(min_app_time)<= '2024-07-25'
and is_owner =1 -- 筛选车主
union all
--活动当日APP日活
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and ta.CREATED_AT >= '2024-07-01' -- 修改月份
...
where date(c.create_time)='2024-07-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-07-25'
	group by member_id
...
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title='7月会员日'
and page_title in ('7月会员日','7月会员日二级页面')
--and var_activity_name='2024年7月会员日'
and var_activity_name in ('2024年7月会员日','2024年会员日组队活动')
and event ='Button_click'
--and m.is_vehicle='1' -- 车主
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and event_time > '2024-07-01' and `date` = '2024-07-25'
	and page_title ='7月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
-- 优惠券核销情况
	...
	where 1=1
	and date(a.get_date)= '2024-07-25'
	and a.is_deleted=0 
	and a.coupon_id in ('7255','7259','7258','7257','7256')--2024.7
--	('6942','6943','6944','6945','6946')--2024.6
...
from 
	...
	where 1=1
	and a.lottery_code like '%memberday-2407%'  -- 当月会员日code
	and date(a.create_time)='2024-07-25'
	and a.have_win = 1   -- 中奖
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-07-25'
...
and date(r.CREATE_TIME) = '2024-06-25'
order by 1
...
-- 抽奖明细
	...
	left join volvo_online_activity_module.lottery_play_init lpi on lpi.lottery_play_code =a.lottery_play_code 
	left join `member`.tc_member_info d on a.member_id = d.ID
	where a.lottery_code like '%memberday-2407%'  -- 当月会员日code
	and date(a.create_time)='2024-07-25'
	and a.have_win = 1   -- 中奖
	--and prize_name like '%2024%'
	order by a.create_time
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='17月会员日'
	and var_activity_name='2023年17月会员日'
	and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-07-25' 
...
-- 解析JSON
SELECT business_code, isValidJSON(business_code) 
FROM ods_dmoa.ods_dmoa_tm_onlineactivity_event_record_d a
...
SELECT
    business_code,
    JSONExtract(business_code, 't1', 'String'),
    JSONExtract(business_code, 't2', 'String'),
    JSONExtract(business_code, 't3', 'String')
FROM ods_dmoa.ods_dmoa_tm_onlineactivity_event_record_d a
where business_code is not null 
and  business_code<>''
