...
(-- 注册会员
	...
	where 1=1
	and m.is_deleted =0 
	and date(m.create_time) = '2024-03-25'
)b 
...
where 1=1
and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App'))
and ((`date` = '2023-01-25' and event= 'Page_view' and page_title ='3月会员日' and subtitle_name = '2023年3月会员日')
or (`date` = '2023-02-25' and event= 'Page_entry' and page_title ='3月会员日'and activity_name = '2023年3月会员日')
...
or (`date` = '2023-11-25' and event= 'Page_entry' and page_title ='13月会员日'and activity_name = '2023年13月会员日'))
and LENGTH(distinct_id)<9
group by distinct_id
) his
...
from (
	...
	,level2_2.rk rk2
		...
		,row_number() over(partition by level1.distinct_id order by level1.date desc) as rk
			...
			and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App') )
			and length(distinct_id)<9 
			and date>=date_sub(date('2024-03-25'),31)
			and date<='2024-03-25'
...
from (
	...
	where date(tpm.create_time)='2024-03-25'
...
and date(a.create_time)='2024-03-25'
...
(-- 到店数大于线索数时,取线索数作为到店数
	...
	(-- 每月每人的到店数
		...
		(-- 一键留资后30天内到店
			...
			(-- 浏览过活动页且 一键留资线索 
				...
				and a.create_time >= '2024-02-25' -- 只要比最小的到店时间小30天即可
			) a on a.customer_mobile = pfi.mobile_phone 
			where 1=1
			and pfi.created_at >= '2024-02-25' 
	...
	(-- 浏览过活动页的 每月每人的一键留资线索
		...
		where a.create_time >= '2024-02-25'  
		and a.create_time < '2024-03-25'
...
where 1=1
and date(min_app_time) = '2024-03-25'
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and month(ta.CREATED_AT) = '3' -- 修改月份
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
where date(c.create_time)='2024-03-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-03-25'
	group by member_id
...
from 
	...
	user_id,
		case 
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=quick_entrance&promotion_activity=202403_wowcar_member&promotion_supplement=%' then '1社区banner'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=quick_entrance&promotion_activity=202403_wowcar_member&promotion_supplement=%' then '2快速入口'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=tweets_btn&promotion_activity=202403_wowcar_member&promotion_supplement=%' then '3公众号推文'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=btn_text_jump&promotion_activity=202403_wowcar_member&promotion_supplement=%' then '4活动推文跳转btn'
			when `$url` like '%promotion_channel_type=sms&promotion_channel_sub_type=sms&promotion_methods=short_link&promotion_activity=202403_wowcar_member&promotion_supplement=%' then '5短信短链'
			when `$url` like '%promotion_supplement=jvuhavyuf%' then '6勋章入口'
...
from 
	...
	where 1=1
	and a.lottery_code like 'memberday-2403'  -- 当月会员日code
	and date(a.create_time)='2024-03-25'
	and a.have_win = 1   -- 中奖
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-03-25'
...
and date(r.CREATE_TIME) = '2024-03-25'
order by 1
...
--	tir.drive_time `实际试驾时间`,
--	x2.created_at,
	case when tir.drive_time>'2000-01-01' then '是' else '否' end `是否试驾`,
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='13月会员日'
	and activity_name='2023年13月会员日'
	and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App') )
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-03-25' 
...
--	x2.created_at,
	case when tad.DRIVE_S_AT>'2000-01-01' then '是' else '否' end `是否试驾`,
	case when toDate(x2.created_at)>=toDate(ta.CREATED_AT) then '是' else '否' end `是否购车`
	FROM ods_cyap.ods_cyap_tt_appointment_d ta
	left join 
		(select tmi.*
		,row_number()over(partition by tmi.cust_id order by tmi.create_time desc) rk 
		...
		where tmi.is_deleted =0
		Settings allow_experimental_window_functions = 1
		)m on ta.ONE_ID = m.cust_id and rk=1
	...
	and left(a.phone_num,1) = '1'
	group by 1
)x2 on x2.phone_num=m.member_phone
	WHERE 1=1
	and date(ta.CREATED_AT) = '2024-03-25'
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
