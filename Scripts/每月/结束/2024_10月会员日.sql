...
and page_title ='10月会员日'
and var_activity_name = '2024年10月会员日'
...
-- 副本二级页推荐购邀约明细 受邀好友完成试驾后完成购车
-- 邀请试驾线索量
select 
count(1) `成功邀请好友完成试驾留资数量`,
count(case when bb.`是否试驾`='已试驾' then 1 end) `受邀好友完成到店试驾数量` ,
count(case when bb.`是否购车` = '是' then 1 end)  `受邀好友完成试驾后完成购车`
from 
(
SELECT  
distinct 
...
-- 组队成功
	select
distinct aa.`distinct_id`
FROM 
(
--_组队通关二级页人群包
SELECT 
	distinct a.distinct_id `distinct_id`,m.id `id`
	from dwd_23.dwd_23_gio_tracking a
	left join ods_gio.ods_gio_user_d b on b.gio_id =a.gio_id 
...
and page_title ='10月会员日二级页面'
--and var_activity_name='2024年9月会员日'
--and var_activity_name in ('2024年9月会员日','2024年会员日组队活动')
and event ='Page_entry'
)aa
inner join
(
...
and a.is_deleted =0
and date(a.create_time) ='2024-10-25'
order by 5 desc 
)bb on toString( aa.id )=toString(bb.`A-memberID（组队发起人）`) 
...
and date(tir.create_time)= '2024-10-25'
...
from 
	...
	join (
		...
		from dwd_23.dwd_23_gio_tracking a
		where 1=1
		and event_time > '2024-10-01' and a.`date` = '2024-10-25'
...
--		and a.`date` <= '2024-05-31'
	...
	where tir.is_deleted=0
	and date(tir.create_time)='2024-09-25'
	and tir.be_invite_member_id is not null 
)x
...
-- 预约试驾
	...
	WHERE 1=1
	and date(ta.CREATED_AT) = '2024-10-25'
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端	
...
where 1=1
and toDate(a.create_time) = '2024-10-25' 
...
and campaign_code in
			...
			and date(concat(left(`_data_inlh_date`,4),'-',right(left(`_data_inlh_date`,6),2),'-',right(`_data_inlh_date`,2))) = '2024-10-30'
				)
...
--					and distinct_id not like '%#%'
				...
				and a.create_time >= '2024-10-25' -- 只要比最小的到店时间小30天即可
			) a on a.customer_mobile = pfi.mobile_phone 
			where 1=1
			and pfi.created_at = '2024-10-25' 
...
where 1=1
and event_time > '2024-10-01' and `date` = '2024-10-25'
and event= '$AppStart'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or  channel='App')
union all 
-- APP当日下载量
...
--APP总车主数
select 4 `rk`,count(1)
from ods_oper_crm.ods_oper_crm_usr_gio_d_si m
where 1=1
and min_app is not null -- 筛选app用户
and date(min_app)<= '2024-10-25'
and is_vehicle ='1' -- 筛选车主
union all
--活动当日APP日活
select 5 `rk`,count(distinct memberid)
from ods_oper_crm.ods_oper_crm_active_gio_d_si 
where platform ='App'
and date(dt)= '2024-10-25'
union all
--当月APP月活
...
FROM ods_invi.ods_invi_tm_invite_record_d tir  
where 1=1
and toDate(tir.create_time)>='2024-10-01'
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and ta.CREATED_AT >= '2024-10-01' -- 修改月份
	and ta.CREATED_AT < '2024-10-01' -- 修改月份
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
where date(c.create_time)='2024-10-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-10-25'
	group by member_id
...
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title='10月会员日'
and page_title in ('10月会员日','10月会员日二级页面')
--and var_activity_name='2024年10月会员日'
and event ='Button_click'
--and m.is_vehicle='1' -- 车主
--and m.is_vehicle='0' -- 粉丝
...
and `date` = '2024-10-25'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title like '%10月会员日%'
and page_title in ('10月会员日','10月会员日二级页面')
--and var_activity_name in ('2024年10月会员日','2024年会员日组队活动')
and event ='Button_click'
group by 1
order by 1
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and event_time > '2024-10-01' and `date` = '2024-10-25'
	and page_title ='10月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
and date(date_create) = '2024-10-25'
...
--	var_promotion_supplement,
	...
	and var_promotion_supplement in ('7n274265a28t',
'2u274373s10q',
'0f274492d11q',
'3g274574w54v',
'0v274693o52l',
...
-- 优惠券核销情况
	...
	where 1=1
	and date(a.get_date)= '2024-10-25'
	and a.is_deleted=0 
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-10-25'
...
and date(r.CREATE_TIME) = '2024-10-25'
order by 1
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='110月会员日'
	and var_activity_name='2023年110月会员日'
	and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-10-25' 
