...
where 1=1
and toDate(a.create_time) = '2024-09-25' 
...
where 1=1
and event_time > '2024-09-01' and `date` = '2024-09-25'
and event= '$AppStart'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or  channel='App')
union all 
-- APP当日下载量
...
and date(min_app_time)<= '2024-09-25'
and is_owner =1 -- 筛选车主
union all
--活动当日APP日活
...
FROM ods_invi.ods_invi_tm_invite_record_d tir  
where 1=1
and toDate(tir.create_time)>='2024-09-01'
...
(-- 回厂
	...
	and date(o.RO_CREATE_DATE) = '2024-08-25'
...
where date(c.create_time)='2024-09-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-09-25'
	group by member_id
...
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title='9月会员日'
and page_title in ('9月会员日','9月会员日二级页面')
--and var_activity_name='2024年9月会员日'
and event ='Button_click'
--and m.is_vehicle='1' -- 车主
--and m.is_vehicle='0' -- 粉丝
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and event_time > '2024-09-01' and `date` = '2024-09-25'
	and page_title ='9月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
and date(date_create) = '2024-09-25'
...
--	var_promotion_supplement,
	...
	and var_promotion_supplement in ('3z261328z32b',
'1j261417y31u',
'4h261517b93q',
'3n261682q15v',
'5w261771d83q',
...
-- 优惠券核销情况
	...
	where 1=1
	and date(a.get_date)= '2024-09-25'
	and a.is_deleted=0 
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-09-25'
...
and date(r.CREATE_TIME) = '2024-09-25'
order by 1
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='19月会员日'
	and var_activity_name='2023年19月会员日'
	and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-09-25' 
