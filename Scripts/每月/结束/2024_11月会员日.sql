...
and page_title ='11月会员日'
and var_activity_name = '2024年11月会员日'
...
from 
	...
	join (
		...
		from dwd_23.dwd_23_gio_tracking a
		where 1=1
		and event_time > '2024-11-01' and a.`date` = '2024-11-25'
		and length(a.distinct_id)<9
		and event = 'Page_entry'
		and page_title='11月会员日'
--		and page_title='11月会员日二级页面'
		and var_activity_name='2024年11月会员日'
		and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
...
where 1=1
and toDate(a.create_time) = '2024-07-25' 
...
(-- 到店数大于线索数时,取线索数作为到店数
	...
	(-- 每月每人的到店数
		...
		(-- 一键留资后30天内到店
			select pfi.id,
			pfi.mobile_phone mobile_phone,
			min(a.create_time) as liuzi
			...
			--		and a.`date` <= '2024-05-31'
					...
					and var_activity_name='2024年1月会员日'
					and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
					)x on toString(x.distinct_id) =toString(m.cust_id) 
				...
				and toDate(a.create_time) = '2024-01-25'  -- 只要比最小的到店时间小30天即可
			) a on a.customer_mobile = pfi.mobile_phone 
			where 1=1
			and pfi.created_at >= '2024-01-25' 
...
where 1=1
and event_time > '2024-11-01' and `date` = '2024-11-25'
and event= '$AppStart'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or  channel='App')
union all 
-- APP当日下载量
...
FROM ods_invi.ods_invi_tm_invite_record_d tir  
where 1=1
and toDate(tir.create_time)>='2024-11-01'
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and ta.CREATED_AT >= '2024-11-01' -- 修改月份
	and ta.CREATED_AT < '2024-11-01' -- 修改月份
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title='11月会员日'
and page_title in ('11月会员日','11月会员日二级页面')
--and var_activity_name='2024年11月会员日'
and event ='Button_click'
--and m.is_vehicle='1' -- 车主
...
and `date` = '2024-11-25'
and ((`$lib` in('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel = 'App')
--and page_title like '%11月会员日%'
and page_title in ('11月会员日','11月会员日二级页面')
--and var_activity_name in ('2024年11月会员日','2024年会员日组队活动')
and event ='Button_click'
group by 1
order by 1
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and event_time > '2024-11-01' and `date` = '2024-11-25'
	and page_title ='11月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android','HarmonyOS') and left($client_version,1)='5') or channel ='App')
--	and is_bind=0
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='111月会员日'
	and var_activity_name='2023年111月会员日'
	and (((`$lib` in ('iOS','Android','HarmonyOS') and left(`$client_version`,1)='5') or channel ='App') or (`$lib` in('MiniProgram') or channel ='Mini'))
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
