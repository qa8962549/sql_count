...
(-- 注册会员
	...
	where 1=1
	and m.is_deleted =0 
	and date(m.create_time) = '2024-04-25'
)b 
...
where 1=1
and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App'))
and ((`date` = '2023-01-25' and event= 'Page_view' and page_title ='4月会员日' and subtitle_name = '2023年4月会员日')
or (`date` = '2023-02-25' and event= 'Page_entry' and page_title ='4月会员日'and activity_name = '2023年4月会员日')
or (`date` = '2023-03-25' and event= 'Page_entry' and page_title ='4月会员日'and activity_name = '2023年4月会员日')
...
or (`date` = '2023-11-25' and event= 'Page_entry' and page_title ='11月会员日'and activity_name = '2023年11月会员日'))
and LENGTH(distinct_id)<9
group by distinct_id
) his
...
from (
	...
	,level2_2.rk rk2
		...
		,row_number() over(partition by level1.distinct_id order by level1.date desc) as rk
			...
			and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App') )
			and length(distinct_id)<9 
			and date>=date_sub(date('2024-04-25'),31)
			and date<='2024-04-25'
...
from (
	...
	where date(tpm.create_time)='2024-04-25'
...
and date(a.create_time)='2024-04-25'
...
from 
	...
	and date(tir.create_time)= '2024-04-25'
...
from 
	...
	join (
		...
		from ods_rawd.ods_rawd_events_d_di a
		where 1=1
		and a.`date` = '2024-03-25'
		and distinct_id not like '%#%'
		and length(a.distinct_id)<9
		and event = 'Page_entry'
		and page_title='3月会员日'
		and activity_name='2024年3月会员日'
		and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App') )
	)x on toString(x.distinct_id) =toString(m.cust_id) 
	where tir.is_deleted=0
	and date(tir.create_time)='2024-03-25'
	and date(tir.create_time)<'2024-04-25'
	and tir.be_invite_member_id is not null 
)x
...
-- 预约试驾
	...
	WHERE 1=1
	and date(ta.CREATED_AT) = '2024-04-25'
...
where 1=1
and toDate(a.create_time) = '2024-04-25' 
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	WHERE 1=1
	and date(ta.CREATED_AT) = '2024-03-25'
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端	
...
(-- 到店数大于线索数时,取线索数作为到店数
	...
	(-- 每月每人的到店数
		...
		(-- 一键留资后30天内到店
			...
			(-- 浏览过活动页且 一键留资线索 
				...
				and a.create_time >= '2024-03-25' -- 只要比最小的到店时间小30天即可
			) a on a.customer_mobile = pfi.mobile_phone 
			where 1=1
			and pfi.created_at >= '2024-03-25' 
			and pfi.created_at < '2024-04-25'
			and a.create_time <= pfi.created_at and toDate(pfi.created_at) <= date_add(day,30,toDate(a.create_time))
	...
	(-- 浏览过活动页的 每月每人的一键留资线索
		...
		where a.create_time >= '2024-03-25'  and a.create_time < '2024-04-25'
...
where 1=1
and date(min_app_time) = '2024-04-25'
--union all
...
--	count(distinct m.member_phone) `预约试驾活动线索（去重）`,
	...
	left join ods_memb.ods_memb_tc_member_info_cur m on toString(ta.ONE_ID)=toString(m.cust_id) 
	WHERE 1=1
	and month(ta.CREATED_AT) = '4' -- 修改月份
	AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
	AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
...
where date(c.create_time)='2024-04-25'
group by is_vehicle
...
from 
	...
	left join ods_voam.ods_voam_activity_card_d ca on c.activity_card_code=ca.activity_card_code 
	where 1=1
	and date(c.create_time)<='2024-04-25'
	group by member_id
...
order by content_title,btn_name
...
--分享星愿数	
	...
	and length(distinct_id)<9 
	and `date` = '2024-04-25'
	and page_title ='4月会员日_一键领取优惠券弹窗'
	and (($lib in('iOS','Android') and left($app_version,1)='5') or channel ='App')
--	and is_bind=0
--	and is_bind=1
	group by btn_name
...
(
	...
	from ods_rawd.ods_rawd_events_d_di 
	where date ='2024-04-25' 
	and page_title='4月会员日'
	and activity_name='2024年4月会员日'
	and event ='Page_entry'
--	and promotion_channel_type in ('app','sms')
--	and is_bind=0
--	and is_bind=1
)x where x.rk=1
and x.promotion_channel_type in ('app','sms')
group by 1,2,3,4,5
order by 1,2,3,4,5
...
from 
	...
	user_id,
		case 
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=app_banner&promotion_activity=202404_wowcar_member&promotion_supplement=%' then '1社区banner'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=quick_entrance&promotion_activity=202404_wowcar_member&promotion_supplement=%' then '2快速入口'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=tweets_btn&promotion_activity=202404_wowcar_member&promotion_supplement=%' then '3公众号推文'
			when `$url` like '%promotion_channel_type=app&promotion_channel_sub_type=app&promotion_methods=btn_text_jump&promotion_activity=202404_wowcar_member&promotion_supplement=%' then '4活动推文跳转btn'
			when `$url` like '%promotion_channel_type=sms&promotion_channel_sub_type=sms&promotion_methods=short_link&promotion_activity=202404_wowcar_member&promotion_supplement=%' then '5短信短链'
			when `$url` like '%promotion_supplement=ibsvaidinbdiv%' then '6勋章入口'
...
--			,b.pay_fee/100 现金支付金额
	...
	where 1=1
	and date(a.get_date)= '2024-04-25'
	and a.is_deleted=0 
	and a.coupon_id in (
	'6676','6677','6678','6679','6680','6681')--2024.4
	--'6374','6375','6376'--2024.3
	--'6311','6312','6313'--2024.2
...
from 
	...
	where 1=1
	and a.lottery_code like 'memberday-2404'  -- 当月会员日code
	and date(a.create_time)='2024-04-25'
	and a.have_win = 1   -- 中奖
...
select *
from volvo_online_activity_module.lottery_draw_log a
where date(a.create_time)='2024-04-25'
...
and date(r.CREATE_TIME) = '2024-04-25'
order by 1
...
-- 抽奖明细
	...
	left join volvo_online_activity_module.lottery_play_init lpi on lpi.lottery_play_code =a.lottery_play_code 
	left join `member`.tc_member_info d on a.member_id = d.ID
	where and a.lottery_code like 'memberday-2403'  -- 当月会员日code
	and date(a.create_time)='2024-04-25'
	and a.have_win = 1   -- 中奖
	--and prize_name like '%2024%'
	order by a.create_time
...
join (
	...
	and length(a.distinct_id)<9
	and event = 'Page_entry'
	and page_title='14月会员日'
	and activity_name='2023年14月会员日'
	and ((`$lib` in('iOS','Android') and left(`$app_version`,1)='5') or `$lib` ='MiniProgram' or  channel in ('Mini', 'App') )
)x on toString(x.distinct_id) =toString(m.cust_id) 
where tir.is_deleted=0
...
WHERE t2.create_time >='2024-04-25' 
...
--	and ca.active_code = 'IBDMAUGMIXSJXCXD2023VCCN'   -- 小程序
    ...
    when tam.IS_TAKE_CAR = 10041002 then '否'
       ...
       case when tam.WORK_ORDER_NUMBER is not null then '是' else '否'end "是否已完成养修"
	from ods_cyap.ods_cyap_tt_appointment_d ta
	left join ods_cyap.ods_cyap_tt_appointment_maintain_d tam on tam.APPOINTMENT_ID =ta.APPOINTMENT_ID 
	left join ods_orga.ods_orga_tm_company_cur tc2 on tc2.company_code =ta.OWNER_CODE 
	left JOIN ods_dict.ods_dict_tc_code_d tc on tc.CODE_ID =tam.MAINTAIN_STATUS 
