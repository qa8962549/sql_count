...
--`开单顾问接触次数`
	...
	tsoe.first_deposit_date `first_deposit_date`
	 ...
	 and tsov.sale_type not in ('20131004','20131003','20131013')-- 订单类型剔除”试驾车”、“代步车”；
	...
	-- and salesPayChannelFlg='10041001'-- 沃尔沃零售支付渠道”为“是” 的订单
	-- and tso.so_no='TYB2025021900001'
	 order by tso.created_at desc)xx 
...
--		and a.CREATED_AT >='2025-02-01' -- 接触时间
--		and a.CREATED_AT <'2025-03-01' -- 接触时间
		union all
		--外呼
		 select
		 distinct 
...
-- 订单详情表 强代理 弱代理
	...
	tsoe.first_deposit_date `first_deposit_date`
	 ...
	 and tsoe.first_deposit_date >='2025-02-01' -- 定金支付时间
	 and tsoe.first_deposit_date <'2025-03-01' -- 定金支付时间
	 and tsov.sale_type not in ('20131004','20131003','20131013')-- 订单类型剔除”试驾车”、“代步车”、“大客户中心订单”；
	 and tso.strong_weak_agent in ('87811003','87811004')--销售模式为“批零制“ “新零售“
	 AND tso.business_type=14031001
	 and (((IFNULL(tso.strong_weak_agent,0)!=87811004 
	         or IFNULL(tsod.direct_dealer_is,0)!=96341002) 
