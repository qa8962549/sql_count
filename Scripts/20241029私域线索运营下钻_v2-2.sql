...
-- 线索（一键留资） liteCRM
	...
	from ods_vced.ods_vced_tm_leads_collection_pool_d tlcp 
		join (select m.member_phone member_phone
		from ods_memb.ods_memb_tc_member_info_cur m 
		where m.member_status <> '60341003' and m.is_deleted =0 
		and m.create_time >='2024-01-01'
		and m.create_time <'2024-10-29'
		and length(m.member_phone)>10
		) x on tlcp.customer_mobile=x.member_phone
	where 1=1
...
from 
	...
	join (select m.member_phone member_phone
		...
		and m.create_time <'2024-10-29'
		and length(m.member_phone)>10
		)x2 on x.mobile=x2.member_phone
...
-- 留资
	...
	join (select m.member_phone member_phone
		...
		and m.create_time <'2024-10-29'
		and length(m.member_phone)>10
		)x2 on a.customer_mobile=x2.member_phone
	where 1=1
...
-- 订单
	...
	FROM ods_cydr.ods_cydr_tt_sales_orders_cur a
		...
		and m.create_time <'2024-10-29'
		and length(m.member_phone)>10
		)x2 on a.purchase_phone=x2.member_phone
...
--		ifnull(tpf.arrive_date,'2025-01-01 00:00:00') `到店时间1`,
--		ifnull(tpf.arrive_date,a2.allot_time) `到店时间2`,
--		abs(toDate(a2.allot_time)-toDate(ifnull(tpf.arrive_date,'2025-01-01 00:00:00'))) dt
--		ROW_NUMBER() over(partition by a.customer_mobile order by tpf.arrive_date) rk
	...
	join (select m.member_phone member_phone
		...
		and m.create_time <'2024-10-29'
		and length(m.member_phone)>10
		)x2 on x.customer_mobile=x2.member_phone
	where 1=1
--	and x.rk=1
...
------------------------首触
-- 私域线索下发前未到店： 私域留资量
	select count(distinct concat(x.customer_mobile,x.`到店时间`))
	from 
		(
		select 
		distinct 
...
from 
	(
		...
		from 
			...
			where 1=1
			and ta.IS_DELETED = 0
			and ta.APPOINTMENT_TYPE in ('70691002','70691001') and ta.DATA_SOURCE='C' and ta.DATA_SOURCE='C' -- 预约试乘试驾
			and ta.CREATED_AT >= '2024-01-01'
			and ta.CREATED_AT < '2024-10-29'
			union all
			select tcc.mobile mobile,
...
left join ods_cypf.ods_cypf_tt_passenger_flow_info_cur tp on x.business_id = tp.customer_business_id and tp.is_deleted = 0 
--where tp.arrive_date>= '2024-01-01' and tp.arrive_date < '2024-10-29' -- 求空值 注释掉
