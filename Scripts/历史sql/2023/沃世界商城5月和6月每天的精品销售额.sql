-- 沃世界商城5月和6月每天的精品销售额，不考虑退款
		select 
		DATE_FORMAT(x.兑换时间,'%Y-%m-%d') "日期",
		...
		(
			...
			where a.create_time >= '2022-05-01' and a.create_time < '2022-07-01'   -- 订单时间
...
b.create_time,
round(b.point_amount/3+b.pay_fee/100,2) 实付金额
from order.tt_order b  -- 订单主表
left join order.tt_order_product  a on a.order_code = b.order_code and b.is_deleted <> 1 -- 订单商品表
where a.spu_type =51121001 
and  b.create_time >= '2021-01-01 00:00:00' 
and b.create_time <= '2022-08-30  23:59:59'   -- 订单时间
and b.is_deleted=0

SELECT 
DATE_FORMAT(a.create_time, '%Y-%m-%d') '日期', round(sum(a.point_amount/3+a.pay_fee/100), 2) '精品销售额'
FROM order.tt_order a  -- 订单主表
left join order.tt_order_product b on a.order_code = b.order_code and b.is_deleted <> 1 -- 订单商品表
left join member.tc_member_info h on a.user_id = h.id and h.is_deleted <> 1 and h.MEMBER_STATUS <> 60341003 -- 会员表(获取会员信息)
WHERE b.spu_type in ('51121001', '51121004') -- 筛选精品
and a.create_time BETWEEN '2022-05-01' and '2022-06-30 23:59:59'
