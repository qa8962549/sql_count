...
(
	...
	ifnull(x.tt,0) 签到天数,
	case when ifnull(x.tt,0)<0.1*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率小于10%'
		when ifnull(x.tt,0)>=0.1*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.2*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率10-20%'
		when ifnull(x.tt,0)>=0.2*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.3*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率20-30%'
		when ifnull(x.tt,0)>=0.3*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.4*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率30-40%'
		when ifnull(x.tt,0)>=0.4*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.5*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率40-50%'
		when ifnull(x.tt,0)>=0.5*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.6*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率50-60%'
		when ifnull(x.tt,0)>=0.6*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.7*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率60-70%'
		when ifnull(x.tt,0)>=0.7*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.8*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率70-80%'
		when ifnull(x.tt,0)>=0.8*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 and ifnull(x.tt,0)<0.9*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率80-90%'
		when ifnull(x.tt,0)>=0.9*DATEDIFF('2022-12-31 23:59:59','2022-01-10')+1 then '满签率90%以上' else null end '签到满签前率',
	m.MEMBER_V_NUM V值余额,
	...
	timestampdiff(year,m.CREATE_TIME,'2023-01-01 00:00:00') 会龄,
	ifnull(x2.tt,0) 会员日参与次数,
	ifnull(x4.最近一次消费间隔,'未下过单'),
	ifnull(x3.下单次数,0),
	ifnull(x3.累计支付V值,0),
	...
	left join 
		...
		from 
			...
			and t.`date` <= '2022-12-31 23:59:59'
...
-- 		,b.coupon_fee/100 优惠券抵扣金额
		...
		where a.create_time < '2023-01-01'   -- 订单时间
...
-- 					and (b.spu_type in (51121001,51121004,51121006,51121007) or (b.spu_type in (51121002,51121003) and a.status<>51031006 )) -- 剔除虚拟商品已关闭订单
				...
				)) x4 on x4.会员id=m.ID where m.IS_DELETED = 0 and m.MEMBER_STATUS <> '60341003'
)x 