	...
	select
	a.MEMBER_ID,
	SUM(a.INTEGRAL) 累计消耗V值量
	from `member`.tt_member_flow_record a
	left join `member`.tc_member_info b on a.MEMBER_ID = b.ID 
	where a.CREATE_TIME <= '2023-04-04' 
	and a.CREATE_TIME >= '2023-03-08' 
	and a.EVENT_TYPE <> 60731025   -- 剔除V值退回
	and a.RECORD_TYPE = 1  -- 0：获取 1：消耗
	and a.EVENT_TYPE <> 60741032 -- 剔除V值过期
	and a.STATUS = 1   -- 有效
	and a.IS_DELETED = 0
	group by 1
	having SUM(a.INTEGRAL)>=300
	order by 2 
	...
	from `member`.tt_member_flow_record a
	left join `member`.tc_member_info b on a.MEMBER_ID = b.ID 
	where a.CREATE_TIME < '2023-04-10' 
	and a.CREATE_TIME >= '2023-03-25' 
	and a.EVENT_TYPE <> 60731025   -- 剔除V值退回
	and a.RECORD_TYPE = 1  -- 0：获取 1：消耗
	and a.EVENT_TYPE <> 60741032 -- 剔除V值过期
	and a.STATUS = 1   -- 有效
	and a.IS_DELETED = 0
	and a.EVENT_DESC not like '%抽奖%'
	group by 1
-- 	having SUM(a.INTEGRAL)>=300
	order by 2 
...
where 
a.create_time >= '2023-03-25' and a.create_time < '2023-04-10'   -- 订单时间
...
-- and b.spu_id in ('9430982')  -- 筛选7种商品
-- and b.spu_name ='355ML 燃油系统清洁剂'
