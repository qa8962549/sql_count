...
select
case when m.MEMBER_SOURCE = '60511001' then '2小程序'
	when m.MEMBER_SOURCE = '60511003' then '1沃尔沃汽车App'
	else '其他' end 用户来源,
...
where t.date >= '2022-02-01' and t.date <= '2022-11-30 23:59:59'    -- 用户访问时间
...
left join (
	...
	join member.tc_member_info m on a.user_id = m.id and m.is_deleted=0 and m.MEMBER_STATUS<>60341003  -- 会员表(获取会员信息)
	where a.create_time < '2021-12-01'  -- 订单时间
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and a.separate_status = '10041002' -- 选择拆单状态否
	and a.status  NOT IN (51031001,51031007) -- 去除预创建和创建失败订单
...
where t.date >= '2022-12-01' and t.date < '2022-12-01'
and (t.typeid = 'XWSJXCX_MALL_HOMEPAGE_V' or json_extract(t.data,'$.embeddedpoint') = 'shop_banner_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'loveCar_shopPart_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'shop_search_CLICK')
GROUP BY 1,2
order by 1
...
from(
 ...
 join (
  ...
  and t.`date` >= '2022-02-01' 
  and t.`date` < '2022-12-01'
  GROUP BY 1,2
...
-- 	,count(DISTINCT t.usertag) 商城UV
	...
	where t.date >= '2021-10-01' and t.date <= '2022-11-30 23:59:59'    -- 用户访问时间
	and (t.typeid = 'XWSJXCX_MALL_HOMEPAGE_V' or json_extract(t.data,'$.embeddedpoint') = 'shop_banner_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'loveCar_shopPart_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'shop_search_CLICK')
-- 	GROUP BY 2
	ORDER BY 2 )a 
left join 
...
from 
	...
	where t.date <'2022-12-01' and t.date >='2022-01-01'    -- 用户访问时间
	and (t.typeid = 'XWSJXCX_MALL_HOMEPAGE_V' or json_extract(t.data,'$.embeddedpoint') = 'shop_banner_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'loveCar_shopPart_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'shop_search_CLICK')
	)a 
left join 
	(
	...
	where t.date >= '2021-02-01' and t.date <'2021-03-01'     -- 用户访问时间
	and (t.typeid = 'XWSJXCX_MALL_HOMEPAGE_V' or json_extract(t.data,'$.embeddedpoint') = 'shop_banner_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'loveCar_shopPart_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'shop_search_CLICK')
...
left join 
	...
	where t.date >= '2022-01-01' and t.date <'2022-01-01'     -- 用户访问时间
	and (t.typeid = 'XWSJXCX_MALL_HOMEPAGE_V' or json_extract(t.data,'$.embeddedpoint') = 'shop_banner_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'loveCar_shopPart_CLICK' or json_extract(t.data,'$.embeddedpoint') = 'shop_search_CLICK')
...
from 
	...
	where a.create_time BETWEEN '2021-10-01' and '2022-11-30 23:59:59'   -- 订单时间
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and a.separate_status = '10041002' -- 选择拆单状态否
	and a.status  NOT IN (51031001,51031007) -- 去除预创建和创建失败订单
...
-- 	where t.date >= DATE_sub(CURDATE()-DAY(CURDATE())+1,INTERVAL 1 MONTH) and t.date < DATE_ADD(CURDATE(),INTERVAL -DAY(CURDATE())+1 DAY)
	where t.date >= '2022-01-01' and t.date <'2022-02-01'
	and t.date>m.member_time 
	GROUP BY 1
) a 
left join (
...
left join (
	...
	from (
		...
		left join (
			...
			join member.tc_member_info m on a.user_id = m.id and m.is_deleted=0 and m.MEMBER_STATUS<>60341003  -- 会员表(获取会员信息)
-- 			where a.create_time < DATE_sub(CURDATE()-DAY(CURDATE())+1,INTERVAL 1 MONTH)  -- 订单时间
			where a.create_time >= '2022-01-01' and a.create_time <'2022-02-01'
			and a.is_deleted <> 1  -- 剔除逻辑删除订单
			and a.type = 31011003  -- 筛选沃世界商城订单
			and a.separate_status = '10041002' -- 选择拆单状态否
			and a.status  NOT IN (51031001,51031007) -- 去除预创建和创建失败订单
