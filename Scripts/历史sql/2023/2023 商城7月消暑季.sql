...
from 
	...
	and a.create_time >= '2023-06-08' and a.create_time <'2023-06-24'   -- 618
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and b.spu_id in 
	('3677',
...
from 
	...
	SELECT 
	distinct 
	case WHEN LEFT(tto.client_id,1) = '6' then '小程序订单'
	 WHEN LEFT(tto.client_id,1) = '2' then 'APP订单' else null end 平台,
	a.id,
	a.one_id,
	a.coupon_id,
	tmi.IS_VEHICLE,
...
--  	WHEN LEFT(a.client_id,1) = '2' then 'APP订单' else null end 平台类型,
	...
	left join "order".tt_order tto on tto.order_code =c.order_code and tto.is_deleted <> 1 -- 订单商品表
	left join (
...
--	'5125'
	...
	and a.get_date >= '2023-07-01'
	and a.get_date < '2023-09-01'
	and a.is_deleted=0 
...
concat(x.coupon_id,x.卡券状态),
count(case when x.IS_VEHICLE=1 then x.id else null end) 车主,
count(case when x.IS_VEHICLE<>1 or x.IS_VEHICLE is null then x.id else null end) 粉丝,
count(x.id)
from 
	(
	SELECT 
...
--	'5125'
	...
	and v.核销时间 >= '2023-08-01'
	and v.核销时间 < '2023-09-01'
--	and a.get_date >= '2023-07-01'
--	and a.get_date< '2023-09-01'
	and a.is_deleted=0 
	order by a.get_date desc 
