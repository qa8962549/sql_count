-- PVUV  2iamw9752z  活动起止时间：2022/6/15—2022/6/21   2022/6/17日  今天下午6点前 中期活动效果（只需提供：活动PV、UV、拉新、激活、预约试驾留资数量
-- 文章明细	
select DATE_FORMAT(o.date_create,'%Y-%m-%d') 			
				,o.ref_id
				,c.title
	...
	and date_create >='2022-08-21' and date_create <='2022-08-31 23:59:59'
	and o.ref_id='2iamw9752z'  
	GROUP BY 1,2,3
	order by 1 

-- 拉新
select 
DATE_FORMAT(l.date_create,'%Y-%m-%d'),
count(DISTINCT m.id)
from 'cms-center'.cms_operate_log l
join member.tc_member_info m on l.user_id=m.user_id and m.is_vehicle=0
where l.date_create >='2022-08-21' and l.date_create <='2022-08-31 23:59:59' 
and l.ref_id='2iamw9752z' and l.type ='VIEW' 
...
where t.`date` >= '2022-08-21' and t.`date` <= '2022-08-31 23:59:59'
and json_extract(t.`data`,'$.path')='/src/pages/common/common-auth/index?returnUrl=%2Fsrc%2Fpages%2Flovecar-package%2Fappointment%2Fappointment%3Fchid%3DTU3O4T41DH%26chcode%3DIBDMAUGMBTIRGYCG2022VCCN%26chtype%3D1'
group by 1
order by 1
	
	
-- PV UV
select case when t.`data` like '%2C46FDD579344FE1ABBD2342A0370B3E%' then '01弹窗'
	when t.`data` like '%859DEE5887DF46C9BC3E6D8400789D9F%' then '02 朋友圈海报'
	when t.`data` like '%6B944542F80A49F3BC67445A7091440A%' then '03 车主群海报'
	when t.`data` like '%9ED6191814F14ECDA4150B1F555C20CA%' then '04 首页置顶轮播bannerr'	
	when t.`data` like '%20652234C5B6419D91E722E36ACACB1A%' then '05 首页活动banner'
  	else null end 分类,
-- tmi.IS_VEHICLE 是否车主,
COUNT(t.usertag) PV,
COUNT(DISTINCT t.usertag) UV
from track.track t
join `member`.tc_member_info tmi on t.usertag = CAST(tmi.USER_ID AS VARCHAR) and tmi.MEMBER_STATUS <> 60341003 and tmi.IS_DELETED = 0
where t.`date` >= '2022-08-09' and t.`date` <= '2022-08-31 23:59:59'
group by 1
order by 1
...
WHERE ta.CREATED_AT >= '2022-08-09'
AND ta.CREATED_AT <= '2022-08-31 23:59:59'
AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
and ca.active_code = 'IBDMAUGMBTIRGYCG2022VCCN'   -- 销售六服 “福”务到位-告别你的油滤
...
-- 预约并成功到店试驾数量
select 
DATE_FORMAT(x.预约时间,'%Y-%m-%d')日期,
count(DISTINCT x.客户ID)
from 
(
SELECT
DISTINCT ta.APPOINTMENT_ID 预约ID,
...
LEFT JOIN (
    ...
    where tm.IS_DELETED = 0 AND tm.COMPANY_TYPE = 15061003 
    ORDER BY tm.COMPANY_CODE ASC) h on h.COMPANY_CODE = ta.owner_code
WHERE ta.CREATED_AT >= '2022-08-21'
AND ta.CREATED_AT <= '2022-08-31 23:59:59'
AND ta.APPOINTMENT_TYPE = 70691002     -- 预约试乘试驾
and tad.status= 70711002 -- 已试驾
AND ta.DATA_SOURCE = 'C'   -- 数据来源B端C端
and ca.active_code = 'IBDMAUGMBTIRGYCG2022VCCN'   -- 销售六服
and ta.CUSTOMER_PHONE not in ('18774245716','18758197483','15237325515','17802166389','13611757536','17521318891')   -- 剔除测试信息
...
from
	...
	where a.create_time >= '2022-08-21'
	and a.create_time <='2022-08-31 23:59:59'
	and c.active_code='IBDMAUGMBTIRGYCG2022VCCN' 
...
join 
	...
	and a.IS_DELETED = 0
	and a.CREATED_AT  >= '2022-08-21'
	and a.CREATED_AT  <='2022-08-31 23:59:59'
...
where teh.object_id = '2iamw9752z'
and teh.create_time >= '2022-08-09'
