...
select 
--b.月份
case when b.商品类型='保养类卡券' then '1售后销售额'
	when b.商品类型='精品' then '2精品销售额'
	else '3其它销售额' end 商品类型
,count(DISTINCT b.会员id) 精品购买人数
--,sum(b.支付V值/3+b.现金支付金额) 销售总金额
,sum(b.实付金额) 销售总金额（实付金额）
,sum(b.总金额) 销售总金额（商品总金额）
-- ,sum(b.支付V值) V值兑换数量
,concat(sum(b.现金支付金额)/sum(b.实付金额)*100,'%') "现金支付比例（现金\实付金额）"
,concat(sum(b.现金支付金额)/sum(b.总金额)*100,'%') "现金支付比例（现金\商品总金额）"
,count(DISTINCT b.订单编号) 总订单数
,count(case when b.现金支付金额 <>0 and b.支付V值=0 then b.订单编号 else null end) 纯现金订单数
	from (
	...
	where a.create_time >= '2023-06-20' and a.create_time <'2023-09-20'   -- 订单时间
...
--	and b.spu_name not like '%售后通用代金券%'
...
,count(case when b.现金支付金额 <>0 and b.支付V值=0 then b.订单编号 else null end) 纯现金订单数
	...
	where a.create_time >= '2022-01-01' and a.create_time <'2023-06-01'   -- 订单时间
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and a.separate_status = '10041002' -- 选择拆单状态否
	and a.status  NOT IN (51031001,51031007) -- 去除预创建和创建失败订单
