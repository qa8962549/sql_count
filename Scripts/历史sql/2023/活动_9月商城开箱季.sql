...
--	and e.退货状态='退款成功' 
	where a.create_time >= '2023-09-26' and a.create_time <'2023-10-07'   -- 订单时间
...
from 
	...
	--and e.退货状态='退款成功' 
	where 1=1
	and a.create_time >= '2023-10-01' and a.create_time <'2023-10-07'   -- 618
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and cv.coupon_id  in 
	('5451',
...
from 
	...
	and a.create_time >= '2023-08-26' and a.create_time <'2023-9-06'   -- 618
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and b.spu_id in 
	('3724',
...
--UGC发帖量 （活动话题发帖量）
select 
count(distinct b.post_id) UGC发帖量
from community.tm_post b
join community.tr_topic_post_link a on a.post_id =b.post_id and b.is_deleted =0
--left join "member".tc_member_info tmi on b.member_id =tmi.id and tmi.is_deleted =0
where  1=1
and b.post_type in ('1007','1001') -- 文章
and a.topic_id='VJMD82LYkY'
--and b.create_time >='2023-09-26'
--and b.create_time <'2023-10-01'
and b.create_time >='2023-10-01'
and b.create_time <'2023-10-07'
...
select a.order_code 订单编号
	...
	from "order".tt_order a  -- 订单主表
	left join "order".tt_order_product b on a.order_code = b.order_code and b.is_deleted <> 1 -- 订单商品表
	left join coupon.tt_coupon_detail cd on a.order_code =cd.order_code  
	left join member.tc_member_info h on a.user_id = h.id and h.is_deleted <> 1  -- 会员表(获取会员信息)
	left join goods.item_spu j on b.spu_id = j.id  and j.front_category_id is not null -- 前台spu表(获取商品前台专区ID)
	left join goods.front_category f on f.id=j.front_category_id -- 前台专区列表(获取前天专区名称)
	left join(
...
--	and e.退货状态='退款成功' 
	where a.create_time >= '2023-10-01' and a.create_time <'2023-10-07'   -- 订单时间
	and a.is_deleted <> 1  -- 剔除逻辑删除订单
	and a.type = 31011003  -- 筛选沃世界商城订单
	and a.separate_status = '10041002' -- 选择拆单状态否
	and a.status  NOT IN (51031001,51031007) -- 去除预创建和创建失败订单
