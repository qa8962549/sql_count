...
join ods_cypf.ods_cypf_tt_passenger_flow_info_cur tp on x.mobile = tp.mobile_phone and tp.is_deleted = 0 
where 1=1
...
and abs(toDate(tso.created_at)-toDate(x.create_time))<=60--60天内订单
...
--		tpf.arrive_date `到店时间`,
		...
		left join ods_cypf.ods_cypf_tt_passenger_flow_info_cur tpf on a2.mobile = tpf.mobile_phone and tpf.is_deleted =0
		where 1=1
		and a.create_time >= '2024-01-01'
		and a.create_time < '2024-10-29'
...
--		join ods_cust.ods_cust_tt_clue_clean_cur tcc on tcc.business_id=ta.CUSTOMER_BUSINESS_ID
		...
		left join ods_cypf.ods_cypf_tt_passenger_flow_info_cur tpf on x.mobile = tpf.mobile_phone and tpf.is_deleted = 0 
