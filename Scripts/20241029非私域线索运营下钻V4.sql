...
from 
	...
	and (tcc.campaign_code not in (
			select distinct trim(hd.code) code
			from ods_oper_crm.ods_oper_crm_umt001_em90_his_d hd
			where trim(hd.channel) = '一键留资'
			and date(concat(left(`_data_inlh_date`,4),'-',right(left(`_data_inlh_date`,6),2),'-',right(`_data_inlh_date`,2))) = '2024-12-31'
		) or tcc.campaign_code is null )
)x
...
--and abs(toDate(x.create_time)- toDate(tp.arrive_date)) <=30 -- 30天内到店
and abs(toDate(tso.created_at)-toDate(x.create_time))<=180--180天内订单
--
...
--3、在这个基础上处理
--如果一个线索既有大于30的也有小于30的，  最终大于30的和空的一起置为2025-02-01，商机id，线索id（线索id union 预约id）,到店时间(时间差—），
--rouw_number(pritition by 线索id order by 到店时间)取 1

	--	abs<30 or daodian.id is null
-- 
--1.线索关联到店（30天内）
--2.计算每一个组合的到店-线索的时间差，匹不到到店填充2025-02-01
--3.min 2的结果
--4.得到线索和到店一对一的数据
...
--		tpf.arrive_date `到店时间`,
		toDateTime(left(CAST( 
						case when abs(toDate(a2.allot_time)-toDate(tpf.arrive_date))<=30 then tpf.arrive_date 
							when abs(toDate(a2.allot_time)-toDate(COALESCE(tpf.arrive_date, '2025-03-01')))>30 then '2025-03-01'
							else null end AS String),19)) arrive_date, -- 大于30天的和空的一起置为2025-02-01
...
select  
	...
	count(distinct x.id)
--	sum(timestampDiff('hour', x.create_time, x.arrive_date))/count(distinct x.id) AS hour_diff -- 平均时长
from 
	(
...
--		tpf.arrive_date `到店时间`,
		toDateTime(left(CAST( 
						case when abs(toDate(x.create_time)-toDate(tpf.arrive_date))<=30 then tpf.arrive_date 
							when abs(toDate(x.create_time)-toDate(COALESCE(tpf.arrive_date, '2025-03-01')))>30 then '2025-03-01'
							else null end AS String),19)) arrive_date, -- 大于30天的和空的一起置为2025-02-01
...
--	sum(timestampDiff('hour', x.arrive_date, x.created_at))/count(distinct x.id) AS hour_diff
from 
	(
	select x.*
	from 
...
--		tpf.arrive_date `到店时间`,
		...
		ROW_NUMBER() over(partition by so_no_id order by 
				case when abs(toDate(x.create_time)-toDate(tpf.arrive_date))<=30 then tpf.arrive_date
				when abs(toDate(x.create_time)-toDate(COALESCE(tpf.arrive_date, '2025-03-01')))>30 then '2025-03-01'
				else null end
			)rk -- 取每个线索对应最早的到店时间
	from 
		...
		and tso.so_status in (14041002,14041003,14041008,14041001)
		and tso.is_deleted = 0 
--		and abs(toDate(x.create_time)- toDate(tpf.arrive_date)) <=30 -- 30天内到店
		and abs(toDate(tso.created_at)-toDate(x.create_time))<=180--180天内订单
		)x where x.rk=1
	)x
group by 1 
...
--		tpf.arrive_date `到店时间`,
		toDateTime(left(CAST( 
						case when abs(toDate(a2.allot_time)-toDate(tpf.arrive_date))<=30 then tpf.arrive_date 
							when abs(toDate(a2.allot_time)-toDate(COALESCE(tpf.arrive_date, '2025-02-01')))>30 then '2025-02-01'
							else null end AS String),19)) arrive_date, -- 大于30天的和空的一起置为2025-02-01
		ROW_NUMBER() over(partition by a.id order by 
				case when abs(toDate(a2.allot_time)-toDate(tpf.arrive_date))<=30 then tpf.arrive_date
				when abs(toDate(a2.allot_time)-toDate(COALESCE(tpf.arrive_date, '2025-02-01')))>30 then '2025-02-01'
...
--		join ods_cust.ods_cust_tt_clue_clean_cur tcc on tcc.business_id=ta.CUSTOMER_BUSINESS_ID
		...
		and tcc.is_deleted=0
		and tcc.create_time>= '2024-01-01'
		and tcc.create_time< '2025-02-01'
