...
left join 
	...
	group by 1,2
	order by 1,2
	)x1 on x1.topic_name=x.topic_name and x1.t=x.t
left join (-- 优质（100个字3张图）篇数
	...
	group by 1,2
	order by 1,2)x2 on x2.topic_name=x.topic_name and x2.t=x.t
left join (-- 话题数据汇总 --互动（转评赞收藏）去重人数
	...
	group by 1,2
	order by 1,2)x3 on x3.topic_name=x.topic_name and x3.t=x.t
left join 
	(
--	话题合集页及对应帖子 UV
	select 
	...
	and t.create_time<'2024-05-06'
	group by 2,1
	order by 2,1)x4 on x4.topic_name=x.topic_name and x4.t=x.t
left join (-- 发帖明细
		select 
		distinct 
		tt.topic_name 话题名称,
...
from 
	...
	select 
--	t,
	concat(date_format(now()-7,'%m/01'),'-',date_format(now()-1,'%m/%d')) t,
	count(distinct x.member_id) num 
	from 
	(
	-- 帖子的参与人数
...
left join 
	...
	date_format(a.create_time,'%Y-%m') t,
	count(distinct a.member_id) "总发帖人数（去重）",
--	count(distinct a.post_id) "总发帖量2",
	count(a.post_id) "总发帖量"
	from community.tm_post a
	left join community.tr_topic_post_link bb on a.post_id =bb.post_id 
	left join community.tm_topic tt on bb.topic_id =tt.topic_id 
...
-- 此刻发帖 2023年总发帖人数（去重），2023年总发帖量
	select 
--	date_format(a.create_time,'%Y-%m') t,
	count(distinct a.member_id) "带话题总发帖人数（去重）",
	count(a.post_id) "带话题总发帖量",
	count(distinct a.post_id) "带话题总发帖量2"
	from community.tm_post a
	left join community.tr_topic_post_link bb on a.post_id =bb.post_id 
	left join community.tm_topic tt on bb.topic_id =tt.topic_id 
